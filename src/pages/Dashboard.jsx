...
import { TrendingUp, Activity, PieChart as PieChartIcon, X, Users, TrendingDown, Wallet, GraduationCap, Loader2 } from 'lucide-react';
...
export default function Dashboard() {
  ...
  const { data: metricas, isLoading: loadingMetricas } = useQuery({
    ...
    initialData: {
      ...
      dadosGraficoPizza: [],
      dadosGraficoBarras: [],
      dadosGraficoMovimentacao: []
    }
  });

  const { data: metricasProfessores } = useQuery({
    queryKey: ['dashboard_professores'],
    queryFn: async () => {
      const res = await fetch(`${API_URL}/api/dashboard/professores`);
      if (!res.ok) throw new Error("Falha ao buscar métricas professores");
      return res.json();
    },
    initialData: { total_professores: 0, professores_ativos: 0 }
  });

  const { data: alertasFrequencia = [] } = useQuery({
    queryKey: ['dashboard_frequencia'],
    queryFn: async () => {
      const res = await fetch(`${API_URL}/api/frequencia-turma`);
      if (!res.ok) return [];
      const dadosFreq = await res.json();
      if (dadosFreq.turma) {
  ...
  // ==========================================
  const viradaMesMutation = useMutation({
    mutationFn: async () => {
      const res = await fetch(`${API_URL}/api/dashboard/forcar-virada-mes`, { method: 'POST' });
      if (!res.ok) {
        const err = await res.json();
        throw new Error(err.erro || 'Erro ao executar virada do mês');
      }
      return res.json();
    },
    onSuccess: () => {
      ...
      queryClient.invalidateQueries(['dashboard_metricas']);
    },
    onError: (erro) => {
      console.error("Erro ao forçar virada de mês:", erro);
      alert(erro.message || 'Erro de conexão ao forçar virada de mês.');
    }
  });

  // ==========================================
  // EFEITOS (Sincronização entre abas)
  // ==========================================
  useEffect(() => {
    const escutarCanal = (evento) => {
      if (evento.data === 'atualizar_dados') {
        queryClient.invalidateQueries(['dashboard_metricas']);
        queryClient.invalidateQueries(['dashboard_professores']);
        queryClient.invalidateQueries(['dashboard_frequencia']);
      }
    };
  ...
  const forcarViradaMes = () => {
    ...
    viradaMesMutation.mutate();
  };

  if (loadingMetricas) {
    return <div className="flex-1 p-4 md:p-8 flex items-center justify-center"><Loader2 className="animate-spin text-emerald-500 w-12 h-12" /></div>;
  }
    ...
    >
      ...
      <div className="relative z-10 p-4 md:p-8">
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            {new Date().getDate() <= 5 && (
              <button
                ...
                disabled={viradaMesMutation.isPending}
              ...
              >
                {viradaMesMutation.isPending ? 'Executando...' : 'Executar Virada do Mês'}
              </button>
            )}
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-6 mb-8">
          <div className="bg-transparent p-5 rounded-xl flex items-center gap-4 transition-all hover:bg-white/5 border border-white/5 shadow-[0_4px_20px_0_rgba(0,0,0,0.2)]">
        ...
        <div className="grid grid-cols-1 gap-6">
          <div className="relative bg-transparent p-6 rounded-xl overflow-hidden border border-white/5 shadow-[0_4px_20px_0_rgba(0,0,0,0.2)]">
              ...
              {alertasFrequencia.length === 0 ? (
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                  {alertasFrequencia.map(aluno => (
                    <div key={aluno.id} className="bg-zinc-900/50 border border-white/5 p-4 rounded-xl flex flex-col gap-3 transition-colors hover:bg-white/5">
        ...
        {modalAberto && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/40 backdrop-blur-md p-4 transition-all duration-300">
            <div className="bg-zinc-950/90 backdrop-blur-xl border border-white/10 rounded-2xl w-full max-w-5xl p-8 relative flex flex-col shadow-[0_8px_32px_0_rgba(0,0,0,0.5)]">
              <button 
              ...
              <div className="h-[60vh] w-full">
                <ResponsiveContainer width="100%" height="100%">
                  ...
                  {modalAberto === 'instrumentos' && (
                    <BarChart
                      layout="vertical"
                      data={metricas.dadosGraficoPizza || []}
                    ...
                    >
                      ...
                      <Bar dataKey="value" radius={[0, 4, 4, 0]} barSize={24}>
                        {(metricas.dadosGraficoPizza || []).map((entry, index) => (
