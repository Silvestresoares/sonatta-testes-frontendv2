...
export default function Responsaveis() {
  ...
  const reenviarEmailAcesso = async (id, nome) => {
    ...
    try {
      ...
      if (resposta.ok) {
        alert(`✅ E-mail de acesso para "${nome || 'Responsável'}" enviado com sucesso!`);
      } else {
        alert(`⚠️ ${dados.erro || 'Erro ao reenviar e-mail.'}`);
      }
    } catch (err) {
  ...
  const reenviarWhatsAppAcesso = async (id, nome, telefone) => {
    ...
    try {
      const resposta = await fetch(`${API_URL}/api/whatsapp/enviar-acesso`, {
        ...
        body: JSON.stringify({ usuarioId: id, tipoUsuario: 'responsavel' })
      });
      const dados = await resposta.json();
      if (resposta.ok) {
        ...
        const linkResp = await fetch(`${API_URL}/api/responsaveis/${id}/gerar-link-senha`, {
          method: 'POST',
          headers: { 'Authorization': `Bearer ${localStorage.getItem('@sonatta:token')}` }
        });
        ...
        if (linkDados.link && fone.length >= 10) {
          const texto = `Olá, ${nome}! 👋\n\nSeu link para criar sua senha e acessar o *Portal da Família* no Sonatta é:\n${linkDados.link}\n\n👤 Login: ${linkDados.email || 'seu CPF'}`;
          window.open(`https://wa.me/55${fone}?text=${encodeURIComponent(texto)}`, '_blank');
        } else {
          alert(`⚠️ ${dados.erro || linkDados.erro || 'Não foi possível enviar por WhatsApp. Verifique se o telefone e e-mail estão cadastrados.'}`);
        }
      }
    } catch (err) {
  ...
  const copiarLinkAcesso = async (id, nome) => {
    ...
    try {
      ...
      if (resposta.ok && dados.link) {
        ...
        alert(`⚠️ ${dados.erro || 'Erro ao gerar link de acesso.'}`);
      }
    } catch (err) {
          ...
          <div className="bg-zinc-900 border border-zinc-800 rounded-xl w-full max-w-md overflow-hidden shadow-2xl flex flex-col" onClick={e => e.stopPropagation()}>
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="pt-2 border-t border-zinc-800">
                ...
                <button
                  type="button"
                  onClick={() => reenviarWhatsAppAcesso(visualizarResponsavel.id, visualizarResponsavel.nome, visualizarResponsavel.telefone)}
                  className="flex items-center justify-center gap-2 w-full bg-[#25D366] hover:bg-[#20bd5a] text-white font-bold py-2 px-4 rounded-lg transition-colors text-xs mt-2 cursor-pointer"
                >
