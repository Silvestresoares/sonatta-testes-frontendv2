...
export default function AgendamentoAulaModal({ isOpen, onClose, initialData = null, tipoPadrao = 'aula_extra', onSaveSuccess }) {
  ...
  const handleSalvarAgendamento = async (e) => {
    ...
    try {
      ...
      if (!resposta.ok) {
        const dados = await resposta.json();
        if (resposta.status === 409 && dados.requerConfirmacao) {
          if (window.confirm(dados.mensagem)) {
            payload.forcarAgendamento = true;
            finalResposta = await fetch(url, {
              method: method,
              headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${token}`
            ...
            if (!finalResposta.ok) {
              const dadosRetry = await finalResposta.json();
              throw new Error(dadosRetry.erro || 'Falha ao agendar aula');
            }
          } else {
            throw new Error('Agendamento cancelado devido a conflito de horário.');
          }
        } else {
