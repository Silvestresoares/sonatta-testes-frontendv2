...
export default function WhatsAppConfig({ token }: { token: string }) {
  ...
  const isConfigured = provider === 'baileys' || (apiUrl && apiKey && instanceName);
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="bg-zinc-950 p-6 rounded-lg border border-zinc-800 space-y-4">
          <div className="flex items-center gap-2 mb-2 text-zinc-300 font-semibold">
            <Link2 size={18} className="text-emerald-500" /> Configuração da API
          </div>
          <p className="text-xs text-zinc-400 mb-4">Selecione e configure a forma de conexão do sistema ao seu número de WhatsApp.</p>
